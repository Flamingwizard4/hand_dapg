...
init_hz = 16
tested_hz = 4
avg_train_rewards_h, avg_test_rewards_h = [], []
for h in [init_hz*(2**x) for x in range(tested_hz)]:
    policy = RNN(e.spec, hidden_size=h, n_layers=job_data['bc_n_layers'], seed=job_data['seed'])
    #policy = MLP(e.spec, hidden_sizes=job_data['policy_size'], seed=job_data['seed'])
    #baseline = MLPBaseline(e.spec, reg_coef=1e-3, batch_size=job_data['vf_batch_size'],
                        #epochs=job_data['vf_epochs'], learn_rate=job_data['vf_learn_rate'])
    ...
    if job_data['algorithm'] != 'NPG':
        ...
        print("========================================")
        demo_paths = pickle.load(open(job_data['demo_file'], 'rb'))
        bc_agent = rnn_BC(demo_paths[:-5], policy, e, epochs=job_data['bc_epochs'], seed=job_data['seed'], batch_size=job_data['bc_batch_size'],
                    lr=job_data['bc_learn_rate'], loss_type='MSE', set_transforms=False)
        in_shift, in_scale, out_shift, out_scale = bc_agent.compute_transformations()
        bc_agent.set_transformations(in_shift, in_scale, out_shift, out_scale)
        bc_agent.set_variance_with_data(out_scale)
        ...
        ts = timer.time()
        print("========================================")
        print("Running %d-hidden size, %d-layer, %d-batch size RNN BC with expert demonstrations for %d epochs"%(h, job_data['bc_n_layers'], job_data['bc_batch_size'], job_data['bc_epochs']))
        print("========================================")
        lox = bc_agent.train()
        print("========================================")
        print("BC training complete !!!")
        ...
        test_score /= len(test_losses)
        p /= len(test_losses)
    with open("rnn_bc_%dhz_%dn_%db_%de_alone_log.txt"%(h, job_data['bc_n_layers'], job_data["bc_batch_size"], job_data['bc_epochs']), 'a') as log_file:
        for lo in lox:
            log_file.write("%f\n"%lo)
        log_file.write("Training performance: %d / %d\n"%(p, len(test_losses)))#(score[0][4], job_data['eval_rollouts']))
        log_file.write("Average training loss: %f\n"%train_score)
    ...
    plt.plot(lox)
    plt.title("RNN BC Alone w/ Hidden Size of %d "%h)
    plt.xlabel("Hidden Size")
    plt.xscale('linear')
    plt.ylabel("Batch Loss")
    plt.legend()
    plt.savefig("rnn_bc_%dhz_%dn_%db_%de_alone_log.png"%(h, job_data['bc_n_layers'], job_data["bc_batch_size"], job_data['bc_epochs']))
    plt.close()
    if job_data['algorithm'] != 'DAPG':
        # We throw away the demo data when training from scratch or fine-tuning with RL without explicit augmentation
        demo_paths = None

    pickle.dump(policy, open('rnn_bc_%dhz_%dn_%db_%de_alone.pickle'%(h, job_data['bc_n_layers'], job_data["bc_batch_size"], job_data['bc_epochs']), 'wb'))
    avg_train_rewards_h.append(train_score)
    avg_test_rewards_h.append(test_score)
    

plt.plot([init_hz*(2**x) for x in range(tested_hz)], avg_train_rewards_h, label="Train Loss")
plt.plot([init_hz*(2**x) for x in range(tested_hz)], avg_test_rewards_h, label="Test Loss")
plt.title("RNN BC Hidden Size vs Test/Train Loss")
plt.xlabel("Hidden Size")
plt.xscale('log')
plt.ylabel("Avg. Loss")
plt.legend()
plt.savefig("rnn_bc_%d-%dhz_alone_graph.png"%(init_hz, init_hz*2**(tested_hz-1)))
plt.close()
'''
...
for t in [init_epochs*(2**x) for x in range(tested_epochs)]:
    policy = RNN(e.spec, hidden_size=job_data['bc_hidden_size'], n_layers=job_data['bc_n_layers'], fc_sizes=tuple([job_data['bc_mlp_hz'] for f in range(2)]), seed=job_data['seed'])
    #policy = MLP(e.spec, hidden_sizes=job_data['policy_size'], seed=job_data['seed'])
    #baseline = MLPBaseline(e.spec, reg_coef=1e-3, batch_size=job_data['vf_batch_size'],
                        #epochs=job_data['vf_epochs'], learn_rate=job_data['vf_learn_rate'])
    ...
    plt.plot(lox)
    plt.title("%d-layer, %d-batch size RNN BC Alone w/ %d Epochs"%(job_data['bc_n_layers'], job_data['bc_batch_size'], t))
    plt.xlabel("Epoch")
    plt.xscale('linear')
    plt.ylabel("Batch Loss")
