...
# v_obs, v_act = pad_paths(test_observations, test_expert_action_paths, max_act_len)
...
# {'c_n': 6.0, 'c_r': 5.0, 'i_f': 6.0, 'i_n': 12.0, 'm_f': 5.0, 's_f': 4.0}
ncp_wiring = kncp.wirings.NCP(
    ...
    recurrent_command_synapses=6,  # How many recurrent synapses are in the command neuron layer
...
v_dataloader = data.DataLoader(
    data.TensorDataset(v_data_x, v_data_y), batch_size=1, shuffle=False, num_workers=4
)
...
trainer = pl.Trainer(
    logger=TensorBoardLogger('tb_logs', name='ncp_model'),#pl.loggers.CSVLogger("log"),
    max_epochs=240,
...
ts = timer.time()
trainer.fit(learn, dataloader, v_dataloader)
print("========================================")
...
# m_trials = MongoTrials('mongo://localhost:1234/ncp_db/jobs', exp_key="exp1")
# trials = Trials()
# ncp_param_space = {
#     's_f': hp.quniform("s_f", 2, 5, 1),
#     'i_n': hp.quniform("i_n", 6, 16, 1),
#     'i_f': hp.quniform("i_f", 2, 6, 1), grow
#     'c_n': hp.quniform("c_n", 6, 12, 1),
#     'c_r': hp.quniform("c_r", 1, 6, 1), grow
